...
open class ColorPickerView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    fileprivate lazy var collectionView: UICollectionView = {
        ...
        let collectionView = UICollectionView(frame: CGRect(x: 0.0, y: 0.0, width: self.frame.size.width, height: self.frame.size.height), collectionViewLayout: layout)
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if style == .circle {
            cell.layer.cornerRadius = cell.bounds.width / 2
             ...
             if cell.backgroundColor == UIColor(hex: "#ffffff") {
...
extension ColorPickerView{
    //MARK: ---- Reset solid color selection--------
    open func resetSelectedSolidColorSelection(index : Int){
           guard selectionStyle == .check else { return }
            let pickerCollectionView = self.collectionView
           if let colorPickerCell : ColorPickerCell = collectionView(pickerCollectionView, cellForItemAt: IndexPath(item: index, section: 0)) as? ColorPickerCell {
                colorPickerCell.checkbox.checkState = .mixed
                colorPickerCell.checkbox.toggleCheckState()
                pickerCollectionView.reloadItems(at: [IndexPath(item: index, section: 0)])
           }
       }
    open func pickerCollectionView() -> UICollectionView?{
          return collectionView
      }
      
      open func lastSelectedIndex() -> Int?{
          return _indexOfSelectedColor
      }
